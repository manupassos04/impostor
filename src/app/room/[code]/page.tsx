  ...
  async function handleAdvanceTurn() {
    ...
    if (nextSeat >= players.length) {
      const { data: fresh } = await supabase
        .from('impostor_players').select('voted_for').eq('room_id', room.id)
      const alreadyVoted = fresh?.every(p => p.voted_for !== null) ?? false
      await supabase.from('impostor_rooms')
        .update({ status: alreadyVoted ? 'result' : 'voting', current_hint_seat: nextSeat })
        .eq('id', room.id)
    } else {
      await supabase.from('impostor_rooms')
